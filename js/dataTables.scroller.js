	 ...
	 *  }
		...
		"_fnScroll": function ()
		{
			...
			if ( iScrollTop < this.s.redrawTop || iScrollTop > this.s.redrawBottom ) {
				...
				else if ( iTopRow + rowsCount > this.s.dt.fnRecordsDisplay() ) {
					/* At the end of the table */
					iTopRow = this.s.dt.fnRecordsDisplay() - rowsCount;
					if ( iTopRow < 0 ) {
						iTopRow = 0;
					}
				}
		...
		"_fnDrawCallback": function ()
		{
			var
				...
				displayLen = this.getVisibleRowsCount(),
		...
		getVisibleRowsCount: function(){
			// get real displayed rows count
			var rowsCount;
			if(!$('#dt-serving-history tbody tr:hidden').length){
				// if all rows are visible use standard method
				rowsCount = this.s.dt._iDisplayLength;
			}else{
				// otherwise calculate from DOM
				rowsCount = $('#dt-serving-history tbody tr:visible').length;
			}
			return rowsCount;
		}
	} );
